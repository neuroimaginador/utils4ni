...
void Average_block(NumericVector ima,int x,int y,int z,int neighborhoodsize,double *average, double weight, IntegerVector vol_size)
{
...
void Value_block(double *Estimate, double *Label,int x,int y,int z,int neighborhoodsize,double *average, double global_sum, IntegerVector vol_size, double hh)
{
  int x_pos,y_pos,z_pos;
  bool is_outside;
  double value = 0.0;
...
double distance(NumericVector ima,int x,int y,int z,int nx,int ny,int nz,int f,int sx,int sy,int sz)
{
...
NumericVector adaptive_nonlocal_means_denoising(NumericVector ima, IntegerVector dims, int v, int f, double h) {
  ...
  int init,Ndims,i,j,k,ii,jj,kk,ni,nj,nk,indice;
  ...
  Estimate = (double*) malloc(dims[0] * dims[1] * dims[2] * sizeof(double));
  Label = (double*) malloc(dims[0] * dims[1] * dims[2] * sizeof(double));
  means = (double*) malloc(dims[0] * dims[1] * dims[2] * sizeof(double));
